...
`Punch-game` 是一个基于 HTML5 Canvas 和 JavaScript 实现的空战射击小游戏。玩家需要操控战机击败不断涌出的敌人，并通过收集道具和升级来提升战力，最终挑战 Boss 获取更高积分。所有进度都会存储在浏览器的 localStorage 中，下次打开页面即可继续游玩。随着 Boss-only「Challenge」模式与挑战专属无敌道具的加入，玩家可以体验更明显的模式差异与策略抉择。
...
## 游戏模式 (Game Modes)
- **普通模式（Normal）**：默认模式会刷新三种不同敌机与道具波次。通过击败敌人获取积分及强化资源，同时可以随时切换场地 Buff，提高生存与输出能力。拾取超级组合可以累计次数，满足条件后在拥有黄金飞机的前提下永久激活，但不会产生无敌效果。
- **Boss 连续战（Boss Rush）**：
...
- **挑战模式（Challenge）**：
  ...
  - 整个挑战过程中不会生成普通敌机，也不会触发普通道具波次，只有挑战专属的「无敌道具」会定时掉落，拾取后短暂无敌，持续时间受升级影响；
  - 作弊码或超级组合在挑战模式下会追加无敌效果，但不会计入永久激活次数，确保模式独占性。
...
- `tq123`：立刻获得 10000 积分。
- `tq456`：触发 20 秒的超级组合效果（双倍火力、攻速和积分）。在挑战模式中会额外附带短暂无敌，但其他模式仅提供三项增益。
...
### v1.7.0 (2025-11-14)
- **挑战模式扩展**：引入专属「挑战无敌道具」，固定间隔掉落并可在强化面板中提升持续时间；拾取后会显示 HUD 倒计时。
- **超级组合机制分流**：
  - 普通/Boss 模式继续累积超级组合次数，达成条件后（黄金飞机持有者）即可永久激活但不再提供无敌；
  - 挑战模式的超级组合仅在当前局生效，同时可与无敌道具叠加但不计入永久次数。
- **作弊码差异化**：`tq456` 仅在挑战模式下附带短暂无敌效果，其余模式只提供三项增益，避免常规模式滥用。
- **黄金飞机体验**：切换状态会被写入 localStorage，死亡或刷新后保持原本选择，减少重复操作。
- **UI & 文案**：强化面板新增挑战道具名称与状态提示，常驻 HUD 可清晰展示超级组合计数与无敌剩余时间。
